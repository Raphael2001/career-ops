...
3. Fill templates/cv-template.html's {{...}} placeholders with the tailored content. Use that template even though modes/pdf.md resolves one via cv-templates.mjs: web runs always use the base template.
4. For each optional section (Core Competencies, Projects, Education, Certifications, Awards & Honors, Skills) that has NO real content for this candidate: DELETE the entire block for that section from the HTML — from its \`<!-- SECTION NAME -->\` comment up to (not including) the next \`<!-- ... -->\` comment — not just the placeholder text inside it. Leaving the \`<div class="section">\` wrapper and its heading behind produces a bare header with nothing under it, which is a defect, not an empty state. Work Experience is the one exception: never drop it even with a thin history.
5. ${CV_ENVELOPE_INSTRUCTION}
...
If NO slug variant resolves, STOP THERE and say so clearly — do not go searching for the company's real careers page or any other lead; the two script runs above are the entire task. Leave portals.yml unchanged in that case. Never touch any other company. This is a config repair: do not submit, send, or click Apply anywhere, and edit no file other than portals.yml.
